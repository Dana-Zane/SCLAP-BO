...
def read_models(pdk_dir: pathlib.Path, config_path=None):
    ...
    try:
        ...
        logger.warning(f"Could not import PDK model generator from {pdk_dir}: {e}. Falling back to built-in models and models.sp")
...
def read_lib(pdk_dir: pathlib.Path,  config_path=None):
    ...
    for subckt in lib_parser.library:
        if isinstance(subckt, SubCircuit):
            has_pdk_generator = False
            try:
                has_pdk_generator = bool(get_generator(subckt.name, pdk_dir))
            except ImportError as e:
                logger.warning(f"Could not import PDK generator for {subckt.name}: {e}. Using template annotations only")
            if has_pdk_generator:
