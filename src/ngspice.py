...
# from . import ngspice_meas as ngmeas

NP_NAN = np.nan if int(np.__version__.split(".", 1)[0]) >= 2 else np.NaN
            ...
            for line in file:
                ...
                try: 
                    if "AC output" in line: 
                        ...
                        vector_idx = 0
                        sim_results[vector_id] = np.full((vector_size, vector_row_size), NP_NAN)
                    elif "NOISE output" in line:
                        content = line.split()
                        if len(content) >= 4:
                            vector_id = "NOISE"
                            vector_size = int(content[-2])
                            vector_row_size = int(content[-1])
                            vector_idx = 0
                            sim_results[vector_id] = np.full((vector_size, vector_row_size), NP_NAN)
                        else:
                            vector_id = None
                        ...
                        if "=" in line:
                            ...
                            parse_vector = True
                        elif parse_vector and vector_id is not None:
                            content = line.split()
                            try:
...
if __name__ == '__main__':
    ...
    for _ in range(10):
        run_simulation("./BAIcO/circuit_examples/folded_cascode_ota/ptm130/cir", "tb_ac.cir", "tb_ac.cir_meas.txt", timeout=2)
        curr_meas = parse_sim_results("./BAIcO/circuit_examples/folded_cascode_ota/ptm130/cir", "tb_ac.cir_meas.txt")
